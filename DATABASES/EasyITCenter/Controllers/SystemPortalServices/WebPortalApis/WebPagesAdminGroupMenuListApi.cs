...
namespace EasyITCenter.Controllers {
    ...
    public class WebPagesAdminGroupMenuListApi : ControllerBase {
        ...
        public async Task<string> GetWebGroupMenuList() {
            ...
            else { return JsonSerializer.Serialize(new ResMsg() { Status = DBResult.error.ToString(), RecordCount = 0, ErrorMessage = DbOperations.DBTranslate("YouDoesNotHaveRights") }); }
        }
        ...
        public async Task<string> InsertOrUpdateGroupWebMenuList([FromBody] WebSettingList1 record) {
            try {
                if (ServerApiServiceExtension.IsAdmin()) {
                    ...
                    if (result > 0) return JsonSerializer.Serialize(new ResMsg() { InsertedId = groupWebMenu.Id, Status = DBResult.success.ToString(), RecordCount = result, ErrorMessage = string.Empty });
