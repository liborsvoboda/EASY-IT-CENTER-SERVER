    ...
    public class EasyITCenterProdGuidGroupListApi : ControllerBase {
        ...
        public async Task<string> InsertGroupList([FromBody] ProdGuidGroupList record) {
            try {
                ...
                if (result > 0) return JsonSerializer.Serialize(new ResMsg() { Status = DBResult.success.ToString(), RecordCount = result, ErrorMessage = string.Empty });
        ...
        public async Task<string> DeleteGroupList(string id) {
            try {
                int Ids;
                if (!int.TryParse(id, out Ids)) return JsonSerializer.Serialize(new ResMsg() { Status = DBResult.error.ToString(), RecordCount = 0, ErrorMessage = "Id is not set" });
