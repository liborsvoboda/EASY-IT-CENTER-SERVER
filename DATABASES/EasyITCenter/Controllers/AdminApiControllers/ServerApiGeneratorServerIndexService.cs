...
namespace EasyITCenter.Controllers {
    ...
    public class ServerApiGeneratorServerIndexService : ControllerBase {
        ...
        [Consumes("application/json")]
        public async Task<IActionResult> GetLibraryFilesIndex(LibraryIndexRequest apiRequest) {

            List<string>? loadedFiles =new(); List<string>? loadedRootFolders =new(); string rootFolder = null;
            if (ServerApiServiceExtension.IsAdmin()) {
                try {
                    switch (apiRequest.SelectedLibrary) {
                        ...
                        case LibraryList.Metro2:
                            rootFolder = Path.Combine(SrvRuntime.SrvHelpPagesPath, "Metro-Docs", "Metro2", "Docs");
                            loadedRootFolders.Add(rootFolder);
                            break;
                        case LibraryList.Metro3:
                            rootFolder = Path.Combine(SrvRuntime.SrvHelpPagesPath, "Metro-Docs", "Metro3", "DocsA");
                            loadedRootFolders.Add(rootFolder);
                            break;
                        case LibraryList.Metro4:
                            rootFolder = Path.Combine(SrvRuntime.SrvHelpPagesPath, "Metro-Docs", "Metro4", "DocsA");
                            loadedRootFolders.Add(rootFolder);
                            break;
                        case LibraryList.Metro5:
                            rootFolder = Path.Combine(SrvRuntime.SrvHelpPagesPath, "Metro-Docs", "Metro5", "Docs");
                            loadedRootFolders.Add(rootFolder);
                            break;
                        case LibraryList.MetroSolution:
                            rootFolder = Path.Combine(SrvRuntime.SrvTemplatesPath, "Metro-Library", "MetroSolutions");
                            loadedRootFolders.AddRange(Directory.EnumerateDirectories(rootFolder));
                            break;
                        case LibraryList.Awesome:
                            rootFolder = Path.Combine(SrvRuntime.SrvHelpPagesPath, "AweSome-Docs");
                            loadedRootFolders.AddRange(Directory.EnumerateDirectories(rootFolder));
                            break;
                        ...
                        case LibraryList.WebPages:
                            rootFolder = Path.Combine(SrvRuntime.SrvWebPagesPath);
                            loadedRootFolders.AddRange(Directory.EnumerateDirectories(rootFolder));
                            break;
                        default: break;
                    }

                    if (apiRequest.OnlyIndexReadmeFile) {
                    ...
                    if (!apiRequest.GetFileListResult) {
                        FileOperations.WriteToFile(Path.Combine(rootFolder, apiRequest.SaveFileName), loadedFiles.OrderBy(a => a).ObjectToJson(), true);
                    } else {
