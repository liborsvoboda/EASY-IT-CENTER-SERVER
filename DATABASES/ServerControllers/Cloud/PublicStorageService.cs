...
namespace EasyITCenter.Controllers {
    ...
    public class PublicStorageService : Controller {
        ...
        public async Task<IActionResult> GetPublicStorage() {
            ...
            try {
                    userRootPath = path = SrvRuntime.SrvPublicPath;
        ...
        public async Task<IActionResult> GetSunImageList() {
            ...
            try {
                ...
                userRootPath = Path.Combine(SrvRuntime.SrvPublicPath, "Images");
        ...
        public async Task<string> DownloadHtmlFromUrl([FromBody] DownloadFileRequest downloadFileRequest) {
            ...
            try {
                if (HttpContextExtension.IsWebAdmin() || HttpContextExtension.IsAdmin()) {
                    ...
                    FileOperations.WriteToFile(Path.Combine(SrvRuntime.SrvPublicPath, "Downloads", downloadFileRequest.Filename), InnerHTML, true);
        ...
        public async Task<string> DownloadPdfFromUrl([FromBody] DownloadFileRequest downloadFileRequest) {
            ...
            try {
                if (HttpContextExtension.IsWebAdmin() || HttpContextExtension.IsAdmin()) {
                    ...
                    await page.EvaluateExpressionHandleAsync("document.fonts.ready");
                    await page.PdfAsync(Path.Combine(SrvRuntime.SrvPublicPath, "Downloads", downloadFileRequest.Filename));
        ...
        public async Task<string> DownloadImageFromUrl([FromBody] DownloadFileRequest downloadFileRequest) {
            ...
            try {
                if (HttpContextExtension.IsWebAdmin() || HttpContextExtension.IsAdmin()) {
                    ...
                    await page.EvaluateExpressionHandleAsync("document.fonts.ready");
                    await page.ScreenshotAsync(Path.Combine(SrvRuntime.SrvPublicPath, "Downloads", downloadFileRequest.Filename));
        ...
        /// <returns></returns>
        [AllowAnonymous]
        [HttpPost("/PublicStorageService/SearchInFiles")]
        [Consumes("application/json")]
        public async Task<IActionResult> SearchInFiles([FromBody] SearchInFilesRequest searchInFilesRequest) {
            ...
            try {
                if (HttpContextExtension.IsLogged()) {
                    ...
                    userRootPath = path = Path.Combine(SrvRuntime.SrvPublicPath, searchInFilesRequest.DataPath);
        ...
        /// </summary>
        /// <param name="copyToUserStorageRequest"></param>
        /// <returns></returns>
        [AllowAnonymous]
        [HttpPost("/PublicStorageService/CopyToUserStorage")]
        [Consumes("application/json")]
        public async Task<string> CopyToUserStorage([FromBody] CopyToUserStorageRequest copyToUserStorageRequest) {
            try {
                if (HttpContextExtension.IsLogged()) {
                    copyToUserStorageRequest.DataPath = copyToUserStorageRequest.DataPath.StartsWith("/") ? copyToUserStorageRequest.DataPath.Substring(1) : copyToUserStorageRequest.DataPath.StartsWith("\\") ? copyToUserStorageRequest.DataPath.Substring(1) : copyToUserStorageRequest.DataPath;

                    if (copyToUserStorageRequest.Filename == null) { FileOperations.CopyDirectory(Path.Combine(SrvRuntime.SrvPublicPath, copyToUserStorageRequest.DataPath), Path.Combine(SrvRuntime.SrvUserPath, HttpContextExtension.GetUserName(), copyToUserStorageRequest.DataPath)); }
                    else { FileOperations.CopyFile(Path.Combine(SrvRuntime.SrvPublicPath, copyToUserStorageRequest.DataPath), Path.Combine(SrvRuntime.SrvUserPath, HttpContextExtension.GetUserName(), copyToUserStorageRequest.DataPath)); }
