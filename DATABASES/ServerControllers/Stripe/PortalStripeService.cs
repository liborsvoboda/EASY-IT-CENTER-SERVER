    ...
    public class PaymentsController : Controller {
        ...
        public ActionResult PaymentIntentService([FromBody] PaymentIntentRequest paymentIntentRequest) {
            var options = new PaymentIntentCreateOptions {
                Amount = paymentIntentRequest.Price * 100,
                Currency = "czk",
        ...
        public ActionResult CreateCheckoutSession([FromBody] CreateCheckoutSessionRequest createCheckoutSessionRequest) {
            SessionCreateOptions? options = new SessionCreateOptions {
                ...
                Mode = "payment",
                UiMode = "embedded",
                ReturnUrl = $"{DbOperations.GetServerParameterLists("ServerPublicUrl").Value}/serverportal?Payed=true",
                //ReturnUrl = $"{DbOperations.GetServerParameterLists("ServerPublicUrl").Value}/serverportal?session_id={HttpContext.Session.Id}",
            };

            SessionService? service = new SessionService();
            Session session = service.Create(options);

            //TODO SET TO THE CASH
        ...
        [HttpGet("/StripeService/SessionStatus")]
