    ...
    public static class CoreOperations {
        ...
        public static HttpContext ChechUrlRequestValidOrAuthorized(HttpContext context) {
            ...
            try {
                ...
                if (validPath == null && routePath.StartsWith("/easydata", StringComparison.OrdinalIgnoreCase)) { routeLayout = RouteLayoutTypes.Metro4Layout; validPath = routePath; routingResult = RoutingActionTypes.Return; }
                if (validPath == null && routePath.StartsWith("/error", StringComparison.OrdinalIgnoreCase)) { routeLayout = RouteLayoutTypes.ErrorLayout; validPath = routePath; routingResult = RoutingActionTypes.Return; }
        ...
        public static string SendEmail(SendMailRequest mailRequest, bool sendImmediately = false) {
            ...
            try {
                if ((!SrvRuntime.DebugMode && !bool.Parse(DbOperations.GetServerParameterLists("ConfigLogWarnPlusToDbEnabled").Value)) || sendImmediately) {
                    ...
                    if (DBConn.DatabaseLogToDbEnabled && mailRequest.Content != null &&
