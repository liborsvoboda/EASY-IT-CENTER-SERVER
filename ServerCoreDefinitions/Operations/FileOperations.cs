...
namespace EasyITCenter.ServerCoreStructure {
    ...
    public static class FileOperations {
        ...
        public static bool CreateFile(string file) {
            ...
            if (!File.Exists(file)) {
                if (Directory.Exists(Path.GetDirectoryName(file))) { File.Create(file).Close(); } else {
        ...
        public static bool CopyFile(string from, string to) {
            string targetDir = string.Empty;
            try {
                if (Directory.Exists(Path.GetDirectoryName(to))) { File.Copy(from, to, true); }
                else {
                    string[] dir = Path.GetDirectoryName(to).Split(Path.DirectorySeparatorChar);
                    dir.ToList().ForEach(directory => {
                        targetDir = string.IsNullOrWhiteSpace(targetDir) ? directory : targetDir + Path.DirectorySeparatorChar + directory;
                        if (!FileOperations.CheckDirectory(targetDir)) { Directory.CreateDirectory(targetDir); }
                    });
        ...
        public static bool MoveFile(string from, string to) {
            string targetDir = string.Empty;
            try {
                if (Directory.Exists(Path.GetDirectoryName(to))) { File.Copy(from, to, true); File.Delete(from); }
                else {
                    string[] dir = Path.GetDirectoryName(to).Split(Path.DirectorySeparatorChar);
                    dir.ToList().ForEach(directory => {
                        targetDir = string.IsNullOrWhiteSpace(targetDir) ? directory : targetDir + Path.DirectorySeparatorChar + directory;
        ...
        public static void CreateDirectory(string directory) {
            string targetDir = string.Empty;
            if (!Directory.Exists(directory)) {
                string[] dir = directory.Replace("/","\\").Split(Path.DirectorySeparatorChar);
                dir.ToList().ForEach(direc => {
                    targetDir = string.IsNullOrWhiteSpace(targetDir) ? direc : targetDir + Path.DirectorySeparatorChar + direc;
                    if (!FileOperations.CheckDirectory(targetDir)) { 
