...
namespace EasyITCenter.ServerCoreStructure {
    ...
    public static class UserStorageOperations {
        ...
        {
            List<string> data = null; List<string> resultData = new(); List<FancyTreeUserData> files = new(); 
            data = FileOperations.GetPathFiles(path, "*", SearchOption.AllDirectories);
            data.ForEach(file => {
                string fileContent = FileOperations.ReadTextFile(file);
                if (fileContent.ToLower().IndexOf(searchInput.ToLower()) > -1) { resultData.Add(file); }
            });
