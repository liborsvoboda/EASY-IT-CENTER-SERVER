...
**For Insert To CUSTOM WEB Project**    
Generate For Solutions, VB/C#/JS/TS/HTML/WPF Project Codes    
> Generate Standalone NETCORE8 Server, Can Be Fully Customize   
> You Can Include Generated Web To Your Project   
> You Need Only Copy Your Project to Scanned Folder    



**Step By Step Manual**   

-  Use **"ProjectBrowser"** Web Generator
1. Generate Solution Or Project, each dll assembly must be writen     
2. Code Copy "Index" Folder Content from Output Folder    
   **Index contain files: 0000000000.html,index.html**   
   To Your Project Some Folder/SubFolder but Target Folder     
3. Copy From FIX "Replace-WebParent" Folder Content   
   to Your New "Index" Folder and Rewrite All   

4. Include DLL Files From FIX "ProjectDll" Folder to Your Project    
    Microsoft.SourceBrowser.Common.dll,   
    Microsoft.SourceBrowser.SourceIndexServer.dll    
   
6. SET Singleton AND Static Path To New Target Folder   

**NetCore Startup.cs**   
```csharp    
    
    var subfolder = Path.Combine(
        ServerRuntimeData.ServerWeb_path,"ProjectBrowser"    
    );   
    if (File.Exists(subfolder)) {   
    services.AddSingleton(new SourceBrowserIndex(subfolder)); }     
    
    app.UseStaticFiles(new StaticFileOptions {   
    RequestPath = "/server-web/CodeBrowser",   
    FileProvider = new PhysicalFileProvider(   
    Path.Combine(ServerRuntimeData.ServerWeb_path, "CodeBrowser"),           
    ExclusionFilters.Sensitive & ~ExclusionFilters.DotPrefixed)   
});   
```   

- Done. Now You can Start Your Project and
