...
//TODO CONFIGURE WEB ROOT PATH HERE 
//Example: "test/web/" = http://server.eu/test/web/ 
let serverWebPath = "server-webpages/projectsbrowser/";
...
function rewriteExternalLink(link) {
    ...
    if (endsWith(url, "/0000000000.html")) {
        var filePath = top.s.location.pathname.slice(1);
        filePath = getDisplayableFileName(filePath);
        filePath = window.location.origin + "/" + serverWebPath + "#" + filePath + anchorSplitChar + link.id;
        link.href = filePath;
        link.onclick = function () {
...
function rewriteSolutionExplorerLink(link) {
    ...
    //if(url.startsWith("/")){url = window.location.origin + "/" + serverWebPath + url.substring(1);}
    ...
    if (setClassName) {
        ...
        if (assembly) {
            if (extension != "ts") {
                link.href = window.location.origin + "/" + assembly + pathname;
            }
        }
    }


    let linkParts = link.href.toLowerCase().split(serverWebPath.toLowerCase());
    link.href = linkParts[0] + serverWebPath.toLowerCase() + linkParts[1] + linkParts[2];
