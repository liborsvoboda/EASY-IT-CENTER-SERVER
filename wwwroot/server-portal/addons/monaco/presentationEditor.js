require.config({ paths: { 'vs': '/server-tools/Editor/monaco/js/monaco-editor/min/vs' } });
require(['vs/editor/editor.main'], function () {
    ...
    addNewEditor("", "PresentationEditor", 'html');

    
    let codeContentEditorLang = document.getElementById('PresentationEditorLang');    
    codeContentEditorLang.onchange = function () {
        monaco.editor.setModelLanguage(Gs.Variables.monacoEditorList.filter(obj => { return obj.elementId == "PresentationEditor" })[0].model, codeContentEditorLang.value);
    }
   
    let codeContentEditorTheme = document.getElementById('PresentationEditorTheme');    
    codeContentEditorTheme.onchange = function () {
        Gs.Variables.monacoEditorList.filter(obj => { return obj.elementId == "PresentationEditor" })[0].editor._themeService.setTheme(codeContentEditorTheme.value)
    }
    ...
    */
    ...
    let selectElement = document.getElementById('PresentationEditorLang');
