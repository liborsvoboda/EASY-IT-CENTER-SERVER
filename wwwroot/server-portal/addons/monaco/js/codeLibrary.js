...
require(['vs/editor/editor.main'], function () {
    ...
    addNewEditor("", "CodeContentEditor", 'javascript');

    
    var codeContentEditorLang = document.getElementById('CodeContentEditorLang');    
    codeContentEditorLang.onchange = function () {
        monaco.editor.setModelLanguage(Gs.Variables.monacoEditorList.filter(obj => { return obj.elementId == "CodeContentEditor" })[0].model, codeContentEditorLang.value);
    }
   
    let codeContentEditorTheme = document.getElementById('CodeContentEditorTheme');    
    codeContentEditorTheme.onchange = function () {
        Gs.Variables.monacoEditorList.filter(obj => { return obj.elementId == "CodeContentEditor" })[0].editor._themeService.setTheme(codeContentEditorTheme.value)
    }
