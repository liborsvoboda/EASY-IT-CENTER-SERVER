...
async function CaptureToImage () {
	setTimeout(async () => {
		let EICvideoCanvas = await async function GoToCanvas();
...
async function GoToCanvas () {
	...
	return new Promise(resolve => {
		EICtmpVideo.addEventListener("canplay", e => {
			let EICtmpCanvas = DrawCaptureImage(EICtmpVideo);
 ...
 * @function
 */
function DrawCaptureImage (video) {
 ...
 * @function
 */
async function StartUserCaptureScreen (filename) {
	...
	mediaRecorder.addEventListener('stop', function () {
		...
		reader.onload () {
			...
			await SaveUserCapturedVideo(filename);
		}
 ...
 * @function
 */
async function StartPublicCaptureScreen (filename) {
	let mediaStream = await navigator.mediaDevices.getDisplayMedia({
		video: true,
		audio: true,
	});
	...
	mediaRecorder.addEventListener('stop', function () {
		...
		reader.onload () {
			var dataURL = reader.result;
			Metro.storage.setItem("CapturedVideo", dataURL);
			await SavePublicCapturedVideo(filename);
		}
		reader.readAsDataURL(blob);
 ...
 * @function
 */
async function StartShareCaptureScreen () {
	try {

		RemoveClass("ShareButton", "success");
		AddClass("ShareButton", "alert");
		document.getElementById("ShareButton").innerHTML = "Stop Share Window";
		setTimeout(function () { document.getElementById("ShareButton").onclick = function () { StopShareCaptureScreen(); } }, 1000);
 ...
 * @function
 */
function StopShareCaptureScreen () {

	RemoveClass("ShareButton", "alert");
	AddClass("ShareButton", "success");
	document.getElementById("ShareButton").innerHTML = "Start Share Window";
	setTimeout(function () { document.getElementById("ShareButton").onclick = function () { StartShareCaptureScreen(); } }, 1000);
 ...
 * @function
 */
function GenerateImageWithCanvas (track, videoElement) {
	const canvas = document.createElement("canvas");
