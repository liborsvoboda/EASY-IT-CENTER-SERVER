...
function CreateToolPanel () {
    ...
    html += '<div class="c-pointer mif-cancel mif-1x icon pos-absolute fg-red" style="top:5px;right:5px;" onclick=function ShowToolPanel(); ></div>';
    ...
    html += '<input id="EnableAutoTranslate" type="checkbox" data-role="checkbox" data-cls-caption="fg-cyan text-bold" data-caption="Auto Translate" onchange=SetUserSettings(); >';
    html += '</div><div class="w-25 mt-1 text-right" style="max-width:25% !important;"><button class="button secondary mini" style="max-width:100% !important;" onclick=CancelTranslation(true); >Cancel Translate</button></div></div>';
    html += '</div>';

    html += '<DIV id=_toolUserSet>';
    html += '<DIV class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12" >';
    html += '<DIV class="form-group m-0 p-0" > <INPUT id=EnableShowDescription onchange=SetUserSettings()  style = "HEIGHT: auto" autocomplete = "off" data-role="checkbox" data-caption="Enable Show Description" > </DIV ></DIV > ';
    html += '<DIV class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12" >';
    html += '<DIV class="form-group m-0 p-0" > <INPUT id=RememberLastJson onchange=SetUserSettings()  style = "HEIGHT: auto" autocomplete = "off" data-role="checkbox" data-caption="Remember Last JSON" > </DIV ></DIV > ';
    html += '<DIV class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12" >';
    html += '<DIV class="form-group m-0 p-0" > <INPUT id=RememberLastHandleBar onchange=SetUserSettings()  style = "HEIGHT: auto" autocomplete = "off" data-role="checkbox" data-caption="Remember Last HandleBar" > </DIV ></DIV > ';
    html += '<DIV class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12" >';
    html += '<DIV class="form-group m-0 p-0" > <INPUT id=EnableScreenSaver onchange=SetUserSettings()  style = "HEIGHT: auto" autocomplete = "off" data-role="checkbox" data-caption="Enable ScreenSaver" > </DIV ></DIV > ';
...
*/
function ShowFavoritesPanel (close) {
...
* @param {boolean} close close
*/
function ShowOnlineToolPanel (close) {
...
*/
function ShowModalMenu (elementId, close = false) {
    if (close || Metro.bottomsheet.isOpen("#" + elementId)) {
        AddClass(elementId, "hidden");
        Metro.bottomsheet.close("#" + elementId);
        AddClass(elementId, "hidden");
    }
    else { Metro.bottomsheet.open("#" + elementId, "grid"); RemoveClass(elementId, "hidden"); }
}
...
function RefreshPreview () {
    RemoveElement("PreviewScript"); RemoveElement("PreviewStyle");
...
//function InfoboxTableCreate (elementId) {
...
*/
function InfoboxFrameCreate (elementId,url) {
...
*/
function InfoboxObjectCreate (elementId, html, width = "80%", height = "800", closeButton = true) {
    let infoBox = Metro.infobox.create(html, "", {
...
*/
function WindowIframeCreate (title, url, lastWindow = false, frameName = "WindowFrame") {
        ...
        place: 'center', customButtons: [
            {
                ...
                cls: "success",
                onclick: `function InfoboxFrameCreate('HelpViewer','${url}Readme', false);`
            },
            {
                html: "<span class='mif-help' title='Show Readme.md Code'></span>",
                cls: "warning",
                onclick: `function InfoboxFrameCreate('HelpCodeViewer','${url}Readme.md', false);`
            },
            {
                html: "<span class='mif-file-code' title='Show Window Code'></span>",
                cls: "alert",
                onclick: "ShowWindowFrameWindowCode()"
            },
            {
                html: "<span class='mif-windows' title='Open in External Window'></span>",
                cls: "sys-button",
                onclick: "function WindowFrameOpenInExternalWindow('External Window','',true);"
            },
            {
...
* @param {string} html html string
*/
function WindowHtmlCreate (elementId, title, html, hidden = false, closeButton = true) {
    let custwindow = Metro.window.create({
        cls: "p-0", title: title, clsCaption: 'bg-orange',
        btnMin: true, btnMax: true, shadow: true,
        draggable: true, resizable: true,
...
*/
function OpenInExternalWindow (title, url, lastWindow = false) {
    if (lastWindow) { (url = document.getElementById("IFrameWindow") != null ? document.getElementById("IFrameWindow").src : "/") }
...
* @param {boolean} lastWindow if LastWindow URL is Taken
*/
function WindowFrameOpenInExternalWindow (url) {
    window.open(url, '_blank');
}
...
async function GetMyQuestionList () {
    ...
    setTimeout(function () {
        ...
        InfoboxObjectCreate("myQuestionList", html);
        loadJS("InjectedScript", scriptCode);
    }, 5000);
}
        ...
        actions: [
            {
                ...
                onclick: function () { ExportTable({ type: 'csv' }); }
            },
            {
                caption: "TXT",
                cls: "js-dialog-close success",
                onclick: function () { ExportTable({ type: 'txt' }); }
            },
            {
                caption: "JSON",
                cls: "js-dialog-close success",
                onclick: function () { ExportTable({ type: 'json', escape: 'true' }); }
            },
            {
                caption: "XLS",
                cls: "js-dialog-close success",
                onclick: function () { ExportTable({ type: 'excel' }); }
            },
            {
                caption: "DOC",
                cls: "js-dialog-close success",
                onclick: function () { ExportTable({ type: 'doc' }); }
            },
            {
                caption: "PDF",
                cls: "js-dialog-close success",
                onclick: function () { ExportTable({ type: 'pdf', jspdf: { orientation: 'l', margins: { right: 10, left: 10, top: 40, bottom: 40 }, autotable: { extendWidth: true } } }); }
            }
        ]
    });
}
...
*/
async function CreateDialogRequest (title, content, actions) {
        ...
        actions: [
            {
                ...
                cls: "js-dialog-close success",
                onclick: function () { PrintOrExportWindow("Download");  }
            },
            {
                caption: "Download Image",
                cls: "js-dialog-close success",
                onclick: function () { PrintOrExportWindow("Image"); }
            },
            {
                caption: "Download Pdf",
                cls: "js-dialog-close success",
                onclick: function () { PrintOrExportWindow("Pdf"); }
            },
            //{
            //    caption: "Copy to Buffer",
            //    cls: "js-dialog-close success",
            //    onclick: function () { PrintOrExportWindow("Copy"); }
            //}
        ]
    });
}
...
function OpenBasket () {
    ...
    dataBasket.forEach(item => {
        ...
        html += `<li><img class='avatar' src='/server-portal/images/logo.png' ><span class='h7' style="margin-left:60px;">${item.Title}</span><span class="second-label">${item.Price} ${item.Currency}</span><span onclick='RemoveItemBasket("${item.Title}")' class="second-action mif-cancel mif-3x fg-red"></span></li>`;
    }); html += `</ul><p></p><div><button class='button warning shadowed' onclick='RunStripe("${price}","${productName}")' ><img src='/server-portal/images/stripe.png'>STRIPE PAY</button></div>`;

    InfoboxObjectCreate("BasketForm", html);
}
...
function DownloadBasket () {
    ...
    dataBasket.forEach(item => {
        ...
        html += `<li><img class='avatar' src='/server-portal/images/logo.png' ><span class='h7' style="margin-left:60px;">${item.Title}</span><span class="second-label">${item.Price} ${item.Currency}</span><span onclick='Gs.Apis.DownloadApi("DashboardService/DownloadLibraryFolder", { Path: "${item.Path.replaceAll('\\', '\\\\')}" }, "${item.Title}", true);DownloadedBasket("${item.Title}");' class="second-action mif-download mif-3x fg-green"></span></li>`;
    }); html += `</ul>`;
...
async function OpenChat () {
    if ($("#ChatPanel").html() == undefined) {
        ...
        let html = `<div class="row"><span class="h3">${Gs.Variables.username}: Chat</span><button onclick="ClearChat()" class="button ml-10 warning shadowed">Clear Chat</button></div><div class="row">`;
        ...
        html += `</div>`;
        InfoboxObjectCreate("ChatPanel", html, width = "470", height = "700");
...
*/
async function OpenShareWindow (selectedUser = null) {
    if ($("#SharePanel").html() == undefined) {
         
        let html = `<div id="SharePanel"><div class="row ml-3"><span class="h3">${Gs.Variables.username}: Share Window Chat</span><select id="userList" data-role="select" data-on-change="ShareUserSelected" data-cls-select="ml-10 w-25" data-clear-button="true" data-add-empty-value="true" data-prepend="Select User" data-empty-value="" ></select>`
        html += `<button id="ShareButton" onclick="Gs.Media.StartShareCaptureScreen()" class="button pos-absolute success shadowed disabled" style="right:140px;">Start Share Window</button><button onclick="ShareClearChat()" class="button pos-absolute warning shadowed" style="right:30px;">Clear Chat</button></div>`;
        ...
        html += `</div></div>`;
        WindowHtmlCreate("SharePanel","Share Desktop & Chat", html);
...
* @function
*/
async function OpenShareReceive () {
    if ($("#ShareReceivePanel").html() == undefined) {
        ...
        html += `<button onclick="ShareReceiveClearChat()" class="button pos-absolute warning shadowed" style="right:30px;">Clear Chat</button></div><div class="row">`;
        ...
        html += ` data-random-color="true" data-cls-message-left="default" data-cls-message-right="bg-cyan fg-white bd-cyan" data-readonly="false" ></div>`;
        html += `</div>`;
        InfoboxObjectCreate("ShareReceivePanel", html, width = "1200", height = "700", true);
...
async function AddSuggest () {
    ...
    <DIV id=TogglePanelBackground class=panel style="MIN-HEIGHT: 700px">
        <DIV class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 mb-0">
            <BUTTON onclick="AddSuggestionSave();" class="button success c-pointer outline shadowed" style="position:absolute; top:20px;right:40px;z-index:2500;">Save Suggestion</BUTTON>
    ...
    InfoboxObjectCreate("AddSuggest", html, width = "1000", height = "800");
...
function EditHelpMenu () {
    let html = `
    <BUTTON onclick="SaveMenuHelp();" class="button success c-pointer outline shadowed" style="position:absolute; top:57px;right:60px;z-index:2500;">Save Menu Help</BUTTON>
    <DIV class="d-flex row gutters ml-5 mr-5 mb-5 border">
    ...
    InfoboxObjectCreate("EditHelpMenu", html, width = "1000", height = "800");
    LoadDataToTool("FastHelpEditor");
}
...
* @function
*/
function TestWindowLink () {
    if ($("#menuNewWindow")[0].checked) {
        window.open($("#menuUrl").val());
    } else {
        function WindowIframeCreate($("#menuName").val(), $("#menuUrl").val());
    }
}
...
async function AddToFavorites () {
    ...
    CreateDialogRequest("Add to Favorites", content, actions);
    setTimeout(function () {
...
* @function
*/
async function RemoveFavorite (menuName,recGuid) {
    ...
    CreateDialogRequest("Remove Favorite", content, actions);
}
...
* @function
*/
async function AddToOnlineToolList () {
    ...
    CreateDialogRequest("Add to Online Tools", content, actions);
    setTimeout(function () {
        $('#menuDescription').summernote({
            tabsize: 2, height: 150, maxHeight: 150,
            lang: 'cs-CZ',
...
* @function
*/
async function RemoveOnlineTool (menuName, recGuid) {
    ...
    CreateDialogRequest("Remove Online Tool", content, actions);
}
...
* @function
*/
async function GenerateRemoveWebSearchList () {
    let webSearch = Metro.storage.getItem('WebSearchList', null);
    let content = `<div>`;
    webSearch.forEach(search => {
        content += `<button id="${search.RecGuid}" class='button alert outline ml-2 mr-2'  onclick="function RemoveWebSearchList('${search.MenuName}','${search.RecGuid}')">Remove ${search.MenuName}</button>`;
    });
    content += `</div>`;
    let actions = [{ caption: "Close", cls: "js-dialog-close alert", onclick: function () { } }]
    CreateDialogRequest("Remove from Web Search", content, actions);
}
...
* @function
*/
async function AddWebSearchList () {
    ...
    CreateDialogRequest("Add to Web Search", content, actions);
    setTimeout(function () {
        $('#menuDescription').summernote({
            tabsize: 2, height: 150, maxHeight: 150,
            lang: 'cs-CZ',
...
* @function
*/
async function RemoveWebSearchList (menuName, recGuid) {
    ...
    CreateDialogRequest("Remove Web Search", content, actions);
}
...
function OpenBrowserConsole () {
    ...
    WindowHtmlCreate("WebBrowserConsole", "Web Console Debugger", html, true, false);
}


//Show Hidden Console
function ShowBrowserConsole () {
    ...
    Gs.Variables.ConsoleLogList.forEach(consoleline => {
        AddWebConsoleLine(consoleline.message, `${consoleline.type}_console`);
    }); 
    ...
    window.addEventListener("error", (event) => { AddWebConsoleLine(event.message, "error_console"); });
}
